...
if __name__ == '__main__':
    ...
    dssp_path = config["dssp_file"]
    
    if not os.path.isfile(dssp_path):
        alt_path = "/usr/local/bin/mkdssp"
        if os.path.isfile(alt_path):
            logging.info(f"dssp file non trovato in {dssp_path}, uso {alt_path} come fallback")
            dssp_path = alt_path
        else:
            logging.error(f"Nessun file DSSP trovato in {dssp_path} o {alt_path}")
            raise FileNotFoundError(f"Nessun file DSSP trovato in {dssp_path} o {alt_path}")
    
    try:
        dssp = dict(DSSP(structure[0], args.pdb_file, dssp=dssp_path))
